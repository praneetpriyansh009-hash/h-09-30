...
async function main() {
  ...
  // 1. Clean existing data (in correct order to respect foreign keys)
  ...
  // 3. Categories (schema requires: name, slug, iconUrl, description, defaultCommissionRate)
  const categoryData = [
  ...
  // 4. Themes (schema requires: name, slug, description, coverImageUrl, moodKeywords)
  const themeData = [
  ...
  const themes: Array<{ id: string; name: string; slug: string }> = [];
  ...
  // 5. Add-Ons (~25 items across 7 categories)
  ...
  // 7. Vendors (8 vendors across categories)
  ...
  const vendorProfiles: Array<{ id: string; userId: string }> = [];
  for (let i = 0; i < 8; i++) {
  ...
  // 8. Bundles, Bookings & Reviews

  // --- Confirmed bundle with confirmed booking ---
  ...
  // A few more standalone reviews for other vendors
  for (let i = 2; i <= 5; i++) {
    if (!allServices[i]) break;
    // Create a dummy completed booking for the review
